...
def create_customers(log: dict[str, list[dict]]) -> list[Customer]:
    """ Returns a list of Customer instances for each customer from the
    input
    ...
    for cust in log['customers']:
        ...
        for line in cust['lines']:
            ...
            if line['contract'] == 'prepaid':
                ...
                contract = PrepaidContract(datetime.date(2017, 12, 25),
                                           100)
...
def new_month(customer_list: list[Customer],
              month: int,
              year: int) -> None:
...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        if event_data['type'] == 'call':
            src_and_dst_num = (event_data['src_number'], event_data[
                'dst_number'])
            ...
            current_call = Call(src_and_dst_num[0], src_and_dst_num[1],
                                call_time,
                                duration,
                                src_loc, dst_loc)

            if call_time.month != current_month or start == 1:
                new_month(customer_list, call_time.month, call_time.year)
                current_month = call_time.month
                start = 0

            src = find_customer_by_number(src_and_dst_num[0], customer_list)
            dst = find_customer_by_number(src_and_dst_num[1],
                                          customer_list)

            src.make_call(current_call)
            dst.receive_call(current_call)
...
if __name__ == '__main__':
    ...
    # Gather all calls to be drawn on screen for filtering, but we only
    # want
    # to plot each call only once, so only plot the outgoing calls to
    # screen.
